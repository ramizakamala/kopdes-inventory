...
@section('content')
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="rounded-lg border border-zinc-200/70 bg-white px-4 py-2 text-sm text-zinc-600 shadow-sm">
            Total estimasi pengadaan: <span class="font-semibold text-zinc-900">{{ $totalRekomendasi }}</span> unit
        </div>
    </div>

    <div class="mb-4 mb-4 rounded-xl border border-zinc-200/70 bg-white px-4 py-3 text-xs text-zinc-500">
        Barang dengan stok ≤ <span class="text-zinc-600">ROP</span> (Reorder Point).
                ...
                <tbody class="divide-y divide-zinc-100">
                    @forelse ($barangs as $b)
                        ...
                        <tr class="transition hover:bg-zinc-50/60">
                            ...
                            <td class="px-5 py-3 {{ $b->stok_saat_ini <= 0 ? 'font-semibold text-red-600' : 'font-semibold text-amber-300' }}">{{ $b->stok_saat_ini }}</td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3 font-semibold text-emerald-300">{{ max(0, ($rop * 2) - $b->stok_saat_ini) }}</td>
