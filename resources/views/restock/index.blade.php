...
@section('content')
    ...
    <div class="mb-4 rounded-xl border border-white/5 bg-white/[0.02] px-4 py-3 text-xs text-zinc-500">
        Barang dengan stok ≤ <span class="text-zinc-300">ROP</span> (Reorder Point).
                ...
                <tbody class="divide-y divide-white/5">
                    @forelse ($barangs as $b)
                        ...
                        <tr class="transition hover:bg-white/[0.02]">
                            ...
                            <td class="px-5 py-3">
                                <span class="font-semibold text-white">{{ $rop }}</span>
                                <span class="block text-[10px] text-zinc-600">LT {{ $b->lead_time_hari }} hr · SS {{ $b->safety_stock }}</span>
                            </td>
                            <td class="px-5 py-3 text-zinc-300">{{ max(0, $rop - $b->stok_saat_ini) }}</td>
